...
public class TileEntityAncientJar extends TileEntityLockableLoot implements ITickable {
	...
	public String getName() {
		return this.hasCustomName() ? this.customName : "Ancient Jar";
	}
	...
	public Container createContainer(InventoryPlayer playerInventory, EntityPlayer playerIn) {
		...
		return new ContainerAncientJar(playerInventory, this, playerIn);
	}

	@Override
	public String getGuiID() {
		
		return Reference.MOD_ID + ":ancient_jar";
	}
	...
	public void update() {
		...
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerAncientJar)
                {
                    if (((ContainerAncientJar)entityplayer.openContainer).getChestInventory() == this)
                    {
